...
// hit：這個行業的「爆發」機會 —— 平常薪水普通，但每年有機率中一次大的。
//   p = 基礎機率，mult = [最少, 最多] 幾倍年薪，stat = 哪個能力會提高中的機率，
//   text = 中了要寫在人生紀錄裡的句子。實際機率還會乘上能力和年資，算法在 engine.js 的 careerHit()。
export const JOBS = [
  ...
  { id: 'sales', name: '業務員', salary: 45 * WAN, edu: 2, req: { charm: 50 }, raise: 1.6, risk: 1.1, alts: [{ req: { int: 60 }, name: '顧問式業務' }], hit: { p: 0.06, mult: [1.5, 4], stat: 'charm', text: '你談成一筆公司十年來最大的單，獎金抽成直接爆表' } },
  ...
  { id: 'engineer', name: '軟體工程師', salary: 80 * WAN, edu: 3, req: { int: 62 }, raise: 1.3, risk: 1.2, hit: { p: 0.03, mult: [2, 5], stat: 'int', text: '你待的公司被併購，手上的股票選擇權一次兌現' } },
  { id: 'analyst', name: '金融分析師', salary: 90 * WAN, edu: 4, req: { int: 66 }, raise: 1.3, risk: 1.2, alts: [{ req: { charm: 70 }, name: '財富管理業務' }], hit: { p: 0.035, mult: [2, 5], stat: 'int', text: '你押對了一檔市場都看錯的標的，公司分你一大筆績效獎金' } },
  { id: 'lawyer', name: '律師', salary: 110 * WAN, edu: 4, req: { int: 71 }, raise: 1.2, risk: 0.4, hit: { p: 0.03, mult: [2.5, 6], stat: 'int', text: '你打贏一件金額很大的案子，成功報酬一次結清' } },
  { id: 'doctor', name: '醫生', salary: 180 * WAN, edu: 5, req: { int: 74 }, raise: 1.0, risk: 0.2, hit: { p: 0.02, mult: [1.5, 3], stat: 'int', text: '你主持的臨床試驗結案，研究獎金和演講邀約一起下來' } },
  ...
  { id: 'idol', name: '偶像', salary: 70 * WAN, edu: 0, req: {}, raise: 1.4, risk: 0, hidden: true, fade: { from: 28, rate: 0.06 }, hit: { p: 0.05, mult: [2, 5], stat: 'charm', text: '你們團的主打歌在國外爆紅，海外演唱會和分紅一次進來' } },
  // 藝人：轉型走戲劇、主持、代言，靠的是資歷和人脈，年紀大反而是加分，薪水會一直往上
  { id: 'artist', name: '藝人', salary: 90 * WAN, edu: 0, req: {}, raise: 1.7, risk: 0.3, hidden: true, hit: { p: 0.05, mult: [2.5, 7], stat: 'charm', text: '你接到一支全球代言，合約金額是年薪的好幾倍' } },
  { id: 'bandmusician', name: '職業樂手', salary: 34 * WAN, edu: 0, req: {}, raise: 1.2, risk: 0, hidden: true, hit: { p: 0.035, mult: [5, 16], stat: 'charm', text: '你隨手寫的一首歌被拿去當電影主題曲，紅遍大街小巷' } },
  // 社團一路練上去才會出現的職業（不會出現在一般求職名單）
  { id: 'probball', name: '職業籃球員', salary: 110 * WAN, edu: 0, req: {}, raise: 1.7, risk: 0.5, retireAge: 36, hidden: true, hit: { p: 0.04, mult: [2.5, 6], stat: 'hp', text: '你被更高層級的聯盟挖角，簽約金一次付清' } },
  { id: 'ballcoach', name: '籃球教練', salary: 70 * WAN, edu: 0, req: {}, raise: 1.1, risk: 0.4, hidden: true, hit: { p: 0.025, mult: [2.5, 6], stat: 'hp', text: '你帶的球隊打進總冠軍賽，獎金加上球團續約金' } },
  { id: 'dancer', name: '職業舞者', salary: 55 * WAN, edu: 0, req: {}, raise: 1.5, risk: 0.6, retireAge: 42, hidden: true, hit: { p: 0.03, mult: [3, 8], stat: 'charm', text: '你被選進天王的世界巡演舞群，整季的酬勞一次結' } },
  { id: 'choreo', name: '編舞師', salary: 95 * WAN, edu: 0, req: {}, raise: 1.4, risk: 0.4, hidden: true, hit: { p: 0.03, mult: [3, 8], stat: 'charm', text: '你編的舞在網路上被上百萬人跳，授權金和商演一起來' } },
  { id: 'researcher', name: '研究員', salary: 95 * WAN, edu: 0, req: {}, raise: 1.5, risk: 0.3, hidden: true, hit: { p: 0.03, mult: [3, 8], stat: 'int', text: '你的研究拿到專利，被一家大公司買斷授權' } },
  { id: 'speaker', name: '講師／名嘴', salary: 75 * WAN, edu: 0, req: {}, raise: 1.7, risk: 0.8, hidden: true, hit: { p: 0.04, mult: [2, 5], stat: 'charm', text: '你的一段演講被剪成短片瘋傳，演講費和出書邀約湧進來' } },
  { id: 'progamer', name: '電競職業選手', salary: 85 * WAN, edu: 0, req: {}, raise: 1.8, risk: 0.6, retireAge: 28, hidden: true, hit: { p: 0.05, mult: [2.5, 8], stat: 'int', text: '你們隊拿下國際賽冠軍，獎金加上贊助分紅' } },
  { id: 'ecoach', name: '電競教練', salary: 70 * WAN, edu: 0, req: {}, raise: 1.2, risk: 0.5, hidden: true, hit: { p: 0.03, mult: [2.5, 6], stat: 'int', text: '你帶的隊伍拿下世界冠軍，教練獎金一次到位' } },
  { id: 'influencer', name: '網紅', salary: 30 * WAN, edu: 0, req: { charm: 66 }, raise: 0, risk: 0, volatile: true, alts: [{ req: { int: 66 }, name: '知識型網紅', stat: 'int' }, { req: { hp: 72 }, name: '運動型網紅', stat: 'hp' }], hit: { p: 0.05, mult: [3, 9], stat: 'charm', text: '你有一支影片突然爆紅，業配接到手軟' } },
  { id: 'athlete', name: '職業運動員', salary: 80 * WAN, edu: 0, req: { hp: 71 }, raise: 1.5, risk: 0.3, retireAge: 38, alts: [{ req: { hp: 66, int: 62 }, name: '運動分析師', noRetire: true }], hit: { p: 0.04, mult: [3, 8], stat: 'hp', text: '你在國際賽事奪牌，獎金加上代言一次進帳' } },
  // 第二批行業
  { id: 'chef', name: '廚師', salary: 40 * WAN, edu: 0, req: { hp: 45 }, raise: 1.1, risk: 1, alts: [{ req: { int: 60 }, name: '餐飲研發主廚' }], hit: { p: 0.025, mult: [3, 8], stat: 'int', text: '你掌廚的餐廳拿到米其林星星，身價整個不一樣了' } },
  { id: 'hairdresser', name: '美髮師', salary: 36 * WAN, edu: 0, req: { charm: 45 }, raise: 1.2, risk: 0.8, alts: [{ req: { int: 56 }, name: '美髮品牌講師' }], hit: { p: 0.025, mult: [4, 9], stat: 'charm', text: '你剪的一個造型在社群爆紅，明星都指名找你' } },
  ...
  { id: 'esports', name: '電競選手', salary: 36 * WAN, edu: 0, req: { int: 45 }, raise: 2, risk: 0.5, retireAge: 30, hit: { p: 0.05, mult: [4, 12], stat: 'int', text: '你在大型賽事一戰成名，獎金和直播合約一起來' } },
  { id: 'realtor', name: '房仲業務', salary: 40 * WAN, edu: 2, req: { charm: 54 }, raise: 1.7, risk: 1.3, alts: [{ req: { int: 62 }, name: '不動產分析師' }], hit: { p: 0.06, mult: [2, 6], stat: 'charm', text: '你把一棟賣不掉三年的豪宅成交了，一筆佣金抵好幾年' } },
  ...
  { id: 'designer', name: '平面設計師', salary: 45 * WAN, edu: 3, req: { charm: 45 }, raise: 1.2, risk: 1, alts: [{ req: { int: 58 }, name: 'UI 設計師' }], hit: { p: 0.03, mult: [3, 7], stat: 'charm', text: '你設計的包裝得了國際設計獎，訂單多到接不完' } },
  ...
  { id: 'architect', name: '建築師', salary: 75 * WAN, edu: 4, req: { int: 64 }, raise: 1.2, risk: 0.8, hit: { p: 0.025, mult: [3, 7], stat: 'int', text: '你的事務所拿下一件指標性的公共建築標案' } },
  ...
  { id: 'baker', name: '麵包師', salary: 34 * WAN, edu: 0, req: { hp: 42 }, raise: 1.1, risk: 0.9, hit: { p: 0.025, mult: [4, 9], stat: 'hp', text: '你做的麵包拿下世界大賽冠軍，訂單排到半年後' } },
  { id: 'barista', name: '咖啡師', salary: 32 * WAN, edu: 0, req: { charm: 42 }, raise: 1.1, risk: 1 },
  { id: 'bartender', name: '調酒師', salary: 38 * WAN, edu: 0, req: { charm: 50 }, raise: 1.3, risk: 1.1, alts: [{ req: { int: 58 }, name: '酒類講師' }], hit: { p: 0.025, mult: [4, 9], stat: 'charm', text: '你調的一杯酒拿下世界冠軍，代言和顧問費跟著來' } },
  { id: 'trainer', name: '健身教練', salary: 42 * WAN, edu: 0, req: { hp: 62 }, raise: 1.4, risk: 0.9, alts: [{ req: { int: 63 }, name: '運動科學教練' }], hit: { p: 0.025, mult: [3, 8], stat: 'hp', text: '你帶的學員拿了健美冠軍，課程和代言接到爆' } },
  { id: 'yogateacher', name: '瑜伽老師', salary: 38 * WAN, edu: 0, req: { hp: 58, charm: 45 }, raise: 1.3, risk: 0.8 },
  { id: 'guide', name: '導遊', salary: 40 * WAN, edu: 2, req: { charm: 54 }, raise: 1.2, risk: 1.4, alts: [{ req: { int: 60 }, name: '文史導覽員' }], hit: { p: 0.025, mult: [3, 7], stat: 'charm', text: '你帶的深度旅遊被媒體報導，包團排到兩年後' } },
  { id: 'translator', name: '翻譯', salary: 48 * WAN, edu: 3, req: { int: 58 }, raise: 1.1, risk: 0.9, hit: { p: 0.025, mult: [4, 10], stat: 'int', text: '你翻的一本書變成年度暢銷書，版稅比年薪還多' } },
  { id: 'reporter', name: '記者', salary: 45 * WAN, edu: 3, req: { int: 54, charm: 50 }, raise: 1.2, risk: 1.2, alts: [{ req: { int: 64 }, name: '調查記者' }], hit: { p: 0.025, mult: [3, 8], stat: 'int', text: '你的調查報導引爆全國，得了新聞獎，還被買去拍成紀錄片' } },
  { id: 'writer', name: '作家', salary: 28 * WAN, edu: 2, req: { int: 56 }, raise: 1.8, risk: 0.3, hit: { p: 0.035, mult: [5, 15], stat: 'int', text: '你三年前寫的那本小說被改編成影集，版稅和授權金一次入帳' } },
  { id: 'photographer', name: '攝影師', salary: 40 * WAN, edu: 2, req: { charm: 50 }, raise: 1.3, risk: 1.1, alts: [{ req: { int: 58 }, name: '商業攝影師' }], hit: { p: 0.03, mult: [3, 8], stat: 'charm', text: '你的一張照片得了國際大獎，被品牌買去當全球主視覺' } },
  { id: 'musicteacher', name: '音樂老師', salary: 42 * WAN, edu: 3, req: { charm: 54 }, raise: 1.1, risk: 0.6, alts: [{ req: { int: 60 }, name: '樂理講師' }], hit: { p: 0.025, mult: [4, 10], stat: 'charm', text: '你寫的教材被全國的音樂班採用，版稅一次入帳' } },
  { id: 'preschool', name: '幼教老師', salary: 38 * WAN, edu: 3, req: { charm: 54 }, raise: 1, risk: 0.5, alts: [{ req: { int: 60 }, name: '幼教課程研發' }] },
  { id: 'vet', name: '獸醫', salary: 90 * WAN, edu: 4, req: { int: 64 }, raise: 1.1, risk: 0.3, hit: { p: 0.02, mult: [3, 7], stat: 'int', text: '你救回一隻很稀有的動物上了新聞，診所預約爆滿' } },
  { id: 'tcm', name: '中醫師', salary: 110 * WAN, edu: 4, req: { int: 68 }, raise: 1.1, risk: 0.2, hit: { p: 0.02, mult: [3, 7], stat: 'int', text: '你開的一帖藥方被寫成書，中醫界都在用' } },
